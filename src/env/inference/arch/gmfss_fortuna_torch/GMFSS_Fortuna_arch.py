...
class Model:
    ...
    def load_model(self, path):
        flownet_path = "{}\\GMFSS_fortuna_flownet.pkl".format(path)
        self.flownet.load_state_dict(torch.load(flownet_path))

        metricnet_path = "{}\\GMFSS_fortuna_metric.pkl".format(path)
        self.metricnet.load_state_dict(torch.load(metricnet_path))

        feat_path = "{}\\GMFSS_fortuna_feat.pkl".format(path)
        self.feat_ext.load_state_dict(torch.load(feat_path))

        fusionnet_path = "{}\\GMFSS_fortuna_fusionnet.pkl".format(path)
...
cwd = os.getcwd()
model_dir = os.path.join(cwd, 'arch', 'gmfss_fortuna_torch', 'models')
